...
public class DomesticWeatherServiceTest {
	...
	public void testGetWeatherValidLongZip() {
		...
		assertEquals(VALID_WIND_DIRECTION, weatherService.getWeather(VALID_LONG_ZIP).getWindDirection());
	...
	public void testGetWeatherCanadaZip() {
		when(weatherDaoMock.getWeatherByZip(VALID_CANADA_ZIP)).thenThrow(new ResourceNotFoundException(1L,"Zipcode not found"));
		weatherService.getWeather(VALID_CANADA_ZIP);
	}
	
	@Test(expected = ResourceNotFoundException.class)
	public void testGetWeatherInvalidZip() {
		when(weatherDaoMock.getWeatherByZip(INVALID_ZIP)).thenThrow(new ResourceNotFoundException(1L,"Zipcode not found"));
		weatherService.getWeather(INVALID_ZIP);
	}
	
	@Test(expected = ResourceNotFoundException.class)
	public void testGetWeatherGibberishZip() {
		when(weatherDaoMock.getWeatherByZip(INVALID_GIBBERISH)).thenThrow(new ResourceNotFoundException(1L,"Zipcode not found"));
